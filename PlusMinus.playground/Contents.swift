...
func plusMinus(arr: [Int]) -> Void {
    ...
    let positiveValue = Float(arr.filter{$0 > 0}.count) / Float(arr.count)
    let negativeValue = Float(arr.filter{$0 < 0}.count) / Float(arr.count)
    let zeroValue = Float(arr.filter{$0 == 0}.count) / Float(arr.count)
    print(String(format: "%.5f", positiveValue))
    print(String(format: "%.5f", negativeValue))
    print(String(format: "%.5f", zeroValue))
}
